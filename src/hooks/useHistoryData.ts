...
export const useHistoryData = (
  ...
  const [filterSelection, setDifficultyFilter] =
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        if (showUnvoted) {
          const { options, total } = await getUnvotedOptions(
            user.id,
            pollId,
            currentPage,
            pageSize,
          ...
          const { votes: newVotes, total } = await getVoteHistory(
            user.id,
            pollId,
            currentPage,
            pageSize,
  ...
  const loadOptionCounts = useCallback(
    async (forceRefresh = false) => {
      if (!user || !pollId) return;

      try {
        const counts = await getCachedOptionCounts(
          user.id,
          pollId,
          forceRefresh,
        );
  ...
  const handleUpdateVote = async (
    ...
    try {
      ...
      setVotes((prevVotes) =>
        prevVotes.map((vote) =>
          vote.option_id === optionId ? { ...vote, filter: newFilter } : vote,
        ),
      );
